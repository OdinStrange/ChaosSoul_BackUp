# S ChaosSoul 진도 현황 (2026-07-01 기준)
...
|**7주차**|회복 아이템 + 던전 맵|**회복 아이템** — 포션 개수(`CurrentPotion`/`MaxPotion=3`) 관리, `UsePotion()`으로 체력(`HealAmount=50`) 회복, HUD `PotionQuantity` 텍스트 연동, 사망 시 초기화, 리스폰 시 HUD 중복 제거 구현. 3D 포션 메시 에셋 존재. **월드 픽업 액터** — `APotionPickup` C++ 클래스 제작(`SphereComponent` 충돌 감지, `UStaticMeshComponent` 메시 표시), `AEnemyBase` 사망 시 확률(`DropChance`) 기반 포션 스폰, `AddPotion()` 함수로 인벤토리 증가 + `OnPotionChanged` 브로드캐스트, 인벤토리 가득 찼을 때 픽업 차단. Blueprint 자식(`BP\\\_PotionPickup`)에서 포션 메시 연결 및 `BP\\\_EnemyBase`에서 드랍 설정. **던전 맵** — 구조물 배치 + Blocking Volume으로 바닥 콜리전 처리, `AEnemyBaseSpawner` C++ 클래스 제작(`AEnemyBase` 전용, `ICombatActivatable` 체인 연결), `OnEnemyDied` 델리게이트로 스포너 소진 감지 → 다음 구역 자동 활성화. `TMap::operator\[]` → `Find()`로 교체해 스폰 시 크래시 수정. **적 HP바** — 타격 시 표시, 적 사망 시 숨김, 게임 시작 시 숨김 처리. **미구현**: 미니맵 없음(소울라이크 장르 특성상 제외)|✅ 완료|
...
|월드 포션 픽업 액터|`APotionPickup` C++ 클래스, `AEnemyBase` 사망 시 확률 드랍, 인벤토리 가득 찼을 때 픽업 차단|
|`AEnemyBase` 전용 스포너|`AEnemyBaseSpawner` C++ 클래스, `ICombatActivatable` 체인으로 구역별 순차 활성화|
|적 HP바 가시성 제어|게임 시작 시 숨김, 타격 시 표시, 적 사망 시 숨김|
